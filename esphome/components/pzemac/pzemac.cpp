...
void PZEMAC::on_modbus_data(const std::vector<uint8_t> &data) {
  ...
  this->p_last_update_time_ = millis();
  ...
  ESP_LOGD(TAG,
           ...
           "Hz, PF=%.2f",
           int(this->address_), voltage, current, active_power, active_energy, this->p_last_energy_sensor,
           active_energy - this->p_last_energy_sensor, frequency, power_factor);
  ...
  if (this->energy_sensor_ != nullptr) {
    if (this->p_last_energy_sensor == 0) {
      this->energy_sensor_->publish_state(active_energy);
      this->p_last_energy_sensor = active_energy;
    } else {
      if (abs(active_energy - this->p_last_energy_sensor) < 1000) {
        this->energy_sensor_->publish_state(active_energy);
        this->p_last_energy_sensor = active_energy;
      } else {
        this->energy_sensor_->publish_state(this->p_last_energy_sensor);
      }
    }
  }
...
void PZEMAC::update() {
  ...
  if (this->get_update_interval() != SCHEDULER_DONT_RUN &&
      (millis() - this->p_last_update_time_) > this->get_update_interval() * 2) {
